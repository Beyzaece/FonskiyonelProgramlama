import {
  ...
  import * as LocationGeocoding from "expo-location-reverse-geocoding";
  ...
  const index = () => {
    ...
    const CheckIfLocationEnabled = async () => {
      ...
      if (!enabled) {
        Alert.alert(
          "Konum Hizmetleri Etkin Değil",
          "Devam etmek için konum hizmetlerinizi etkinleştirin",
          [{ text: "Tamam" }],
          { cancelable: false }
        );
      } else {
    ...
    const GetCurrentLocation = async () => {
      ...
      if (status !== "granted") {
        Alert.alert(
          "İzin Verilmedi",
          "Uygulamanın konum hizmetlerini kullanmasına izin verin service",
          [{ text: "Tamam" }],
          { cancelable: false }
        );
      }
